...
LLVM의 배열 선언방식과 유사합니다. (영향을 받은건 아님)
...
`:[int int]`와 `:[int 2]`는 같습니다. Nok lang는 사용방식이 같으면 똑같은 걸로 취급합니다. <br> 
...
```
구현 한계상 Key는 immut 타입만 지원할 수 있습니다. (내가 만들때 얘기)


## 클래스 (Class) - 미정
클래스 기반 언어를 생각 중이나, <br>
알다시피 클래스는 구현이 매우 어렵고 귀찮아서 좀 획기적으로 나타내는 법을 찾는 중 입니다.<br>
없다면 Rust나 Go처럼 할 예정
